...
async fn re_introspecting_manually_overwritten_mapped_field_name(api: &TestApi) {
    ...
    assert_eq_json(&warnings, "[{\"code\":8,\"message\":\"These fields were enriched with @map information taken from the previous Prisma schema.\",\"affected\":[{\"model\":\"User\",\"field\":\"test\"}]}]");
}
...
async fn re_introspecting_mapped_field_name(api: &TestApi) {
    ...
    let warnings = api.re_introspect_warnings(input_dm).await;
    assert_eq_json(&warnings, "[{\"code\":8,\"message\":\"These fields were enriched with @map information taken from the previous Prisma schema.\",\"affected\":[{\"model\":\"User\",\"field\":\"id_1\"},{\"model\":\"User\",\"field\":\"index\"},{\"model\":\"User\",\"field\":\"unique_1\"}]}]");
}
...
async fn re_introspecting_mapped_enum_name(api: &TestApi) {
    ...
    assert_eq_json(&warnings, "[{\"code\":9,\"message\":\"These enums were enriched with @@map information taken from the previous Prisma schema.\",\"affected\":[{\"enm\":\"color\"}]}]");
}
