...
pub trait IntrospectionDatabase: Send + Sync + 'static {
...
pub struct IntrospectionDatabaseWrapper {
    pub database: Arc<dyn IntrospectionDatabase + Send + Sync + 'static>,
}

impl sql_schema_describer::SqlConnection for IntrospectionDatabaseWrapper {
    fn query_raw(
        &self,
        sql: &str,
        schema: &str,
...
impl IntrospectionDatabase for Sqlite {
    fn execute(&self, db: &str, q: Query) -> prisma_query::Result<Option<Id>> {
...
impl IntrospectionDatabase for PostgreSql {
...
impl IntrospectionDatabase for Mysql {
    fn execute(&self, _: &str, q: Query) -> prisma_query::Result<Option<Id>> {
        self.with_connection(|conn| conn.execute(q))
    }
