...
pub(crate) fn custom_assert(left: &str, right: &str) {
    let parsed_expected = datamodel::parse_datamodel(&right).unwrap().datamodel;
...
pub(crate) fn custom_assert_with_config(left: &str, right: &str) {
    let parsed_expected_datamodel = datamodel::parse_datamodel(&right).unwrap().datamodel;
    let parsed_expected_config = datamodel::parse_configuration(&right).unwrap();
    let reformatted_expected = datamodel::render_datamodel_and_config_to_string(
        &parsed_expected_datamodel,
        &parsed_expected_config.configuration,
    )
