...
async fn re_introspecting_custom_compound_id_names(api: &TestApi) -> TestResult {
    ...
    final_dm.assert_eq(&re_introspected);
...
#[test_connector(tags(Postgres, Mssql))]
async fn re_introspecting_custom_compound_id_names_when_id_is_mapped(api: &TestApi) -> TestResult {
    api.barrel()
        .execute(|migration| {
            migration.create_table("User", |t| {
                t.add_column("first", types::integer());
    ...
    let input_dm = api.dm_with_sources(
        r#"
         ...
         model User2 {
             ...
             @@id([first, last], name: "compound", map: "previousname")
         }
     "#,
    );
