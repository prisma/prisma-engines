...
async fn multiple_changed_relation_names(api: &TestApi) -> TestResult {
    ...
    let expected = expect![[r#"
        ...
        datasource db {
          provider = "mysql"
          url      = "mysql://root:prisma@localhost:3307/multiple_changed_relation_names"
        }
    ...
    expected.assert_eq(&api.re_introspect(input_dm).await?);
...
async fn mapped_model_and_field_name(api: &TestApi) -> TestResult {
    ...
    let expected = expect![[r#"
        ...
        datasource db {
          provider = "mysql"
          url      = "mysql://root:prisma@localhost:3307/mapped_model_and_field_name"
        }
