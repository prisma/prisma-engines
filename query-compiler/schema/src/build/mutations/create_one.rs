...
fn filter_checked_create_fields(
    ...
    model.fields().filter_all(move |field| {
        match field {
            // Scalars must be writable, not an autogenerated ID, and not a generated (computed) column.
            ModelField::Scalar(sf) => !sf.is_auto_generated_int_id() && !sf.is_read_only() && !sf.is_generated_column(),
