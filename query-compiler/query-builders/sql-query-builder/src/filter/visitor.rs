...
fn convert_json_filter(
    ...
    let condition: Expression = match *condition {
        ...
        cond => match query_mode {
            QueryMode::Default => default_scalar_filter(expr_json, cond, slice::from_ref(field), alias, ctx),
            QueryMode::Insensitive => insensitive_scalar_filter(
                // We need to stringify here since `insensitive_scalar_filter` assumes that it
                // can use string operations on the expression.
                stringify(expr_json).into(),
                cond,
                slice::from_ref(field),
                alias,
                false,
                ctx,
            ),
...
fn prisma_value_to_like_expression(pv: PrismaValue) -> Expression<'static> {
    match pv {
        PrismaValue::String(s) => Value::text(s).into(),
        PrismaValue::Json(s) => serde_json::from_str::<serde_json::Value>(s.as_str())
            .map(|json| Value::json(json).into())
            // The JSON value is generated by us via JSON.stringify, so this should not be
            // reachable by users and indicates a bug if it ever happens.
            .unwrap_or_else(|_| panic!("Invalid JSON value for LIKE pattern: {s}")),
        PrismaValue::Placeholder(PrismaValuePlaceholder { name, .. }) => {
            Value::opaque(Placeholder::new(name), OpaqueType::Text).into()
        }
