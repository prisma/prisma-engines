...
use super::*;
use crate::{query_document::*, query_graph::*, schema::*};
...
impl<'a> QueryGraphBuilder<'a> {
    ...
    pub fn build(self, operation: Operation) -> QueryGraphBuilderResult<QueryGraph> {
    ...
    fn build_internal(
        ...
        if field_pair.schema_field.query_info().is_some() {
            ...
            if let Some(path) = &*PRISMA_DOT_PATH {
                ...
                std::fs::write(
                    format!("{path}/{current}_{}.graphviz", root_object.name()),
                    graph.to_graphviz(),
                )
                .unwrap();
            }
    ...
    fn dispatch_build(&self, field_pair: FieldPair<'a>) -> QueryGraphBuilderResult<QueryGraph> {
        ...
        let mut graph = match (
            ...
            (QueryTag::FindUniqueOrThrow, Some(m)) => {
                read::find_unique_or_throw(parsed_field, m, query_schema).map(Into::into)
            }
            (QueryTag::FindFirst, Some(m)) => read::find_first(parsed_field, m, query_schema).map(Into::into),
            (QueryTag::FindFirstOrThrow, Some(m)) => {
                read::find_first_or_throw(parsed_field, m, query_schema).map(Into::into)
            }
            ...
            (QueryTag::CreateOne, Some(m)) => {
                QueryGraph::root(|g| write::create_record(g, query_schema, m, parsed_field))
            }
            (QueryTag::CreateMany, Some(m)) => {
                QueryGraph::root(|g| write::create_many_records(g, query_schema, m, false, parsed_field))
            }
            (QueryTag::CreateManyAndReturn, Some(m)) => {
                QueryGraph::root(|g| write::create_many_records(g, query_schema, m, true, parsed_field))
            }
            (QueryTag::UpdateOne, Some(m)) => {
                QueryGraph::root(|g| write::update_record(g, query_schema, m, parsed_field))
            }
            (QueryTag::UpdateMany, Some(m)) => {
                QueryGraph::root(|g| write::update_many_records(g, query_schema, m, false, parsed_field))
            }
            (QueryTag::UpdateManyAndReturn, Some(m)) => {
                QueryGraph::root(|g| write::update_many_records(g, query_schema, m, true, parsed_field))
            }
            (QueryTag::UpsertOne, Some(m)) => {
                QueryGraph::root(|g| write::upsert_record(g, query_schema, m, parsed_field))
            }
            (QueryTag::DeleteOne, Some(m)) => {
                QueryGraph::root(|g| write::delete_record(g, query_schema, m, parsed_field))
            }
            (QueryTag::DeleteMany, Some(m)) => {
                QueryGraph::root(|g| write::delete_many_records(g, query_schema, m, parsed_field))
            }
            ...
            (QueryTag::FindRaw, m) => {
                QueryGraph::root(|g| write::query_raw(g, m, Some(QueryTag::FindRaw.to_string()), parsed_field))
            }
            (QueryTag::AggregateRaw, m) => {
                QueryGraph::root(|g| write::query_raw(g, m, Some(QueryTag::AggregateRaw.to_string()), parsed_field))
            }
            (QueryTag::RunCommandRaw, m) => {
                QueryGraph::root(|g| write::query_raw(g, m, Some(QueryTag::RunCommandRaw.to_string()), parsed_field))
            }
