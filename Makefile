...
start-cockroach:
	...
	docker exec -d prisma-engines_cockroach_1 cockroach sql --insecure -e "CREATE USER prisma;"
	docker exec -d prisma-engines_cockroach_1 cockroach sql --insecure -e "GRANT admin TO prisma;"
	docker exec -d prisma-engines_cockroach_1 cockroach sql --insecure -e "ALTER USER prisma SET null_ordered_last = true;"
	...
	docker exec -d prisma-engines_cockroach_1 cockroach sql --insecure -e "SET CLUSTER SETTING kv.range_merge.queue_interval = '50ms';"
	docker exec -d prisma-engines_cockroach_1 cockroach sql --insecure -e "SET CLUSTER SETTING kv.raft_log.disable_synchronization_unsafe = true;"
	docker exec -d prisma-engines_cockroach_1 cockroach sql --insecure -e "SET CLUSTER SETTING jobs.retention_time = '15s';"
	docker exec -d prisma-engines_cockroach_1 cockroach sql --insecure -e "SET CLUSTER SETTING jobs.registry.interval.cancel = '180s';"
	docker exec -d prisma-engines_cockroach_1 cockroach sql --insecure -e "SET CLUSTER SETTING jobs.registry.interval.gc = '30s';"
	docker exec -d prisma-engines_cockroach_1 cockroach sql --insecure -e "SET CLUSTER SETTING sql.stats.automatic_collection.enabled = false;"
	docker exec -d prisma-engines_cockroach_1 cockroach sql --insecure -e "ALTER RANGE default CONFIGURE ZONE USING gc.ttlseconds = '5';"
	docker exec -d prisma-engines_cockroach_1 cockroach sql --insecure -e "ALTER DATABASE system CONFIGURE ZONE USING gc.ttlseconds = '5';"
