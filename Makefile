...
build-driver-adapters: ensure-prisma-present
	...
	@if [ -d ../prisma ]; then \
		...
		echo "git clone --depth=1 https://github.com/LucianBuzzo/prisma.git --branch=$(DRIVER_ADAPTERS_BRANCH) ../prisma"; \
		git clone --depth=1 https://github.com/LucianBuzzo/prisma.git --branch=$(DRIVER_ADAPTERS_BRANCH) "../prisma" && echo "Prisma repository has been cloned to ../prisma"; \
	fi;
