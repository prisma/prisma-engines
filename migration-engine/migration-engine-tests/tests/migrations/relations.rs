    ...
    assert_eq!(
        ...
        &[
            ForeignKey {
                ...
                referenced_table: if api.lower_case_identifiers() { "b" } else { "B" }.to_string(),
            ...
            ForeignKey {
                constraint_name: match api.sql_family() {
                    ...
                    SqlFamily::Mysql if api.lower_case_identifiers() => Some("a_ibfk_2".to_owned()),
                    SqlFamily::Mysql => Some("A_ibfk_2".to_owned()),
                    SqlFamily::Sqlite => None,
                    SqlFamily::Mssql => Some("A_cid_fkey".to_owned()),
                },
                columns: vec![c_column.name.clone()],
                referenced_table: if api.lower_case_identifiers() { "c" } else { "C" }.to_string(),
                referenced_columns: vec!["id".to_string()],
    ...
    assert_eq!(
        ...
        &[ForeignKey {
            constraint_name: match api.sql_family() {
                ...
                SqlFamily::Sqlite => None,
                SqlFamily::Mysql if api.lower_case_identifiers() => Some("b_ibfk_1".to_owned()),
                SqlFamily::Mysql => Some("B_ibfk_1".to_owned()),
                SqlFamily::Mssql => Some("B_a_id_fkey".to_owned()),
            },
            columns: vec!["a_id".to_string()],
            referenced_table: if api.lower_case_identifiers() { "a" } else { "A" }.to_string(),
            referenced_columns: vec!["id".to_string()],
            on_delete_action: ForeignKeyAction::Cascade,
            on_update_action: ForeignKeyAction::NoAction,
        }]
