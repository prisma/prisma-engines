...
/// Drop the database configured in the passed in schema.
pub async fn drop_database(prisma_schema: &str) -> CoreResult<()> {
    ...
    match &source.active_provider {
        provider if [MYSQL_SOURCE_NAME, POSTGRES_SOURCE_NAME, SQLITE_SOURCE_NAME].contains(&provider.as_str()) => {
            SqlMigrationConnector::drop_database(&source.url().value).await?;
        }
        x => unimplemented!("Connector {} is not supported yet", x),
    }
