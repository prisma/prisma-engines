use crate::tests::test_harness::sql::TestApi;
use quaint::prelude::Queryable;
use sql_schema_describer::SqlSchema;
use std::sync::Arc;

pub(crate) struct BarrelMigrationExecutor<'a> {
    pub(crate) api: &'a TestApi,
    pub(crate) sql_variant: barrel::backend::SqlVariant,
}

impl BarrelMigrationExecutor<'_> {
    pub(crate) async fn execute<F>(self, migration_fn: F) -> anyhow::Result<SqlSchema>
    where
