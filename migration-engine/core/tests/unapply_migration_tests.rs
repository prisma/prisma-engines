...
async fn unapply_must_work(api: &TestApi) {
    ...
    let result1 = api.infer_and_apply(&dm1).sql_schema;
    ...
    let result2 = api.infer_and_apply(&dm2).sql_schema;
    assert!(result2.table_bang("Test").column("field").is_none());

    let result3 = api.unapply_migration().sql_schema;
    assert_eq!(result1, result3);

    // reapply the migration again
    let result4 = api.infer_and_apply(&dm2).sql_schema;
