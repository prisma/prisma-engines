...
pub(crate) trait SqlSchemaCalculatorFlavour {
    fn calculate_enums(&self, _datamodel: &ValidatedSchema<'_>) -> Vec<sql::Enum> {
        Vec::new()
    }
    ...
    fn default_constraint_name(&self, _default_value: DefaultValueWalker<'_, '_>) -> Option<String> {
        None
    }

    fn default_native_type_for_scalar_type(&self, scalar_type: &ScalarType) -> serde_json::Value;

    fn enum_column_type(&self, _field: ScalarFieldWalker<'_, '_>, _db_name: &str) -> sql::ColumnType {
        unreachable!("unreachable enum_column_type")
    }

    fn field_is_implicit_autoincrement_primary_key(&self, _field: ScalarFieldWalker<'_, '_>) -> bool {
        false
    }

    fn m2m_foreign_key_action(
        &self,
        _model_a: ModelWalker<'_, '_>,
        _model_b: ModelWalker<'_, '_>,
