...
fn views_can_be_described(api: TestApi) {
    ...
    let expected_sql;
    if api.connector_tags().contains(TiDB) {
        expected_sql = format!(
...
// Exclude TiDB, because the index name generated by TiDB for the foreign key is not consistent with MySQL.
#[test_connector(tags(Mysql), exclude(TiDB))]
