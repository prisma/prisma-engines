...
fn diffing_postgres_schemas_when_initialized_on_sqlite(mut api: TestApi) {
    ...
    api.diff(DiffParams {
        ...
        from: DiffTarget::SchemaDatamodel(vec![SchemaContainer {
            file_path: from_file.to_string_lossy().into_owned(),
            schema: from_schema.to_string(),
        }]),
        shadow_database_url: None,
        to: DiffTarget::SchemaDatamodel(vec![SchemaContainer {
            file_path: to_file.to_string_lossy().into_owned(),
            schema: to_schema.to_string(),
        }]),
        script: true,
    })
    .unwrap();
...
fn from_schema_datasource_relative(mut api: TestApi) {
    ...
    let params = DiffParams {
        exit_code: None,
        from: DiffTarget::SchemaDatasource(vec![SchemaContainer {
            file_path: schema_path.to_string_lossy().into_owned(),
...
fn diffing_two_schema_datamodels_with_missing_datasource_env_vars() {
    for provider in ["sqlite", "postgresql", "postgres", "mysql", "sqlserver"] {
        ...
        expected.assert_eq(&diff_output(DiffParams {
            exit_code: None,
            from: DiffTarget::SchemaDatamodel(vec![SchemaContainer {
                file_path: schema_a_path.to_str().unwrap().to_owned(),
                schema: schema_a.to_string(),
            }]),
            script: false,
            shadow_database_url: None,
            to: DiffTarget::SchemaDatamodel(vec![SchemaContainer {
                file_path: schema_b_path.to_str().unwrap().to_owned(),
...
fn diff_with_non_existing_sqlite_database_from_datasource() {
    ...
    let error = diff_error(DiffParams {
        ...
        script: false,
        shadow_database_url: None,
        to: DiffTarget::SchemaDatasource(vec![SchemaContainer {
            file_path: schema_path.to_string_lossy().into_owned(),
            schema: schema.to_string(),
        }]),
    });
