...
const BLOCKING_TASKS_SHUTDOWN_TIMEOUT: Duration = Duration::from_millis(200);
...
async fn async_main() {
    ...
    tokio::select! {
        ...
        _ = interrupt => {
            ...
            work.await.expect("main task panicked");
        }
...
async fn start_engine(datamodel_locations: Option<Vec<String>>, shutdown_token: CancellationToken) {
    ...
    // Block the thread and handle IO in async until EOF or cancelled.
    tokio::select! {
