...
#[user_facing(
    code = "P3011",
    message = "The migration `{migration_name}` can not be rolled back since it was never applied to the database."
)]
...
#[user_facing(
    code = "P3012",
    message = "The migration `{migration_name}` can not be rolled back since it is not in a failed state."
)]
