...
export interface SchemaEngine {
  new (params: SchemaEngineParams, adapter: SqlDriverAdapter): SchemaEngine
...
export async function initSchemaEngine(
  ...
  const { getSchemaEngineForProvider: getEngineForProvider } = await import(
    './schema-engine-wasm'
  )
  const WasmSchemaEngine = (await getEngineForProvider(
    adapter.provider,
  )) as SchemaEngine
