    ...
    */
    ...
    for row in rows.into_iter() {
        let sql2 = format!(
            ...
            "#, 
            table = row.get("table_name").unwrap().to_string().unwrap(), 
            fk = row.get("constraint_name").unwrap().to_string().unwrap(),
        );
    ...
    let sql_tables = format!(
        r#"
           SELECT TABLE_NAME AS table_name FROM information_schema.TABLES WHERE TABLE_SCHEMA = "{db_name}"
        "#,
        db_name = db_name,
        );
