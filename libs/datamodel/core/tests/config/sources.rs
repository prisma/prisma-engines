use crate::common::*;
use datamodel::{ast::Span, errors_and_warnings::DatamodelError, StringFromEnvVar};
...
"#;
    ...
    let errors_and_warnings = parse_error(&schema);
    errors_and_warnings.assert_length(2);
    errors_and_warnings.assert_is_at(
        0,
...
fn must_forbid_env_functions_in_provider_field() {
    ...
    let errors_and_warnings = config.err().expect("This must error");
    errors_and_warnings.assert_is(DatamodelError::new_functional_evaluation_error(
...
fn must_error_for_empty_urls() {
    ...
    assert!(config.is_err());
    let errors_and_warnings = config.err().expect("This must error");
    errors_and_warnings.assert_is(DatamodelError::new_source_validation_error(
...
fn must_error_for_empty_provider_arrays() {
    ...
    assert!(config.is_err());
    let errors_and_warnings = config.err().expect("This must error");
    errors_and_warnings.assert_is(DatamodelError::new_validation_error(
...
fn must_error_if_env_var_is_missing() {
    ...
    let errors_and_warnings = result.err().unwrap();
    errors_and_warnings.assert_is(DatamodelError::new_environment_functional_evaluation_error(
        "DATABASE_URL",
        Span::new(75, 94),
    ));
}
...
fn fail_to_load_sources_for_invalid_source() {
    ...
    if let Err(errors_and_warnings) = res {
        errors_and_warnings.assert_is(DatamodelError::DatasourceProviderNotKnownError {
...
fn fail_when_no_source_is_declared() {
    ...
    if let Err(errors_and_warnings) = res.configuration.validate_that_one_datasource_is_provided() {
        errors_and_warnings.assert_is(DatamodelError::ValidationError {
