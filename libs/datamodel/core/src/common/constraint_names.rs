use crate::common::source_context::SourceContext;
use crate::IndexType;
...
impl ConstraintNames {
    ...
    pub fn primary_key_name_matches(constraint_name: Option<String>, table_name: &str, ctx: &SourceContext) -> bool {
    ...
    pub fn primary_key_name(table_name: &str, ctx: &SourceContext) -> Option<String> {
        // if ctx.preview_features.contains(&NamedConstraints) {
        if false {
            let suffix = "_pkey";
    ...
    pub fn index_name(table_name: &str, column_names: Vec<String>, tpe: IndexType, ctx: &SourceContext) -> String {
        // if ctx.preview_features.contains(&NamedConstraints) {
        if false {
    ...
    pub fn foreign_key_constraint_name(table_name: &str, column_names: Vec<String>, ctx: &SourceContext) -> String {
        // if ctx.preview_features.contains(&NamedConstraints) {
        if false {
            let fk_suffix = "_fkey";
            let limit = ctx.connector.constraint_name_length();
    ...
    // pub fn default_constraint_name(table_name: &str, column_name: &str, ctx: &SourceContext) -> String {
    //     // if ctx.preview_features.contains(&NamedConstraints) {
    //     if false {
