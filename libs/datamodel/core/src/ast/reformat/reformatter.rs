...
pub struct Reformatter<'a> {
    input: &'a str,
    missing_fields: Result<Vec<MissingField>, crate::messages::ErrorCollection>,
    missing_field_attributes: Result<Vec<MissingFieldAttribute>, crate::messages::ErrorCollection>,
}
...
impl<'a> Reformatter<'a> {
    ...
    fn find_all_missing_fields(schema_string: &str) -> Result<Vec<MissingField>, crate::messages::ErrorCollection> {
    ...
    fn find_all_missing_attributes(
        schema_string: &str,
    ) -> Result<Vec<MissingFieldAttribute>, crate::messages::ErrorCollection> {
        let schema_ast = crate::parse_schema_ast(&schema_string)?;
        let datamodel = crate::parse_datamodel_and_ignore_datasource_urls(&schema_string)?;
        let lowerer = crate::transform::dml_to_ast::LowerDmlToAst::new(None);
