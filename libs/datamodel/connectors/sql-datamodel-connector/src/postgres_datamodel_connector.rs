...
impl Connector for PostgresDatamodelConnector {
    ...
    fn parse_native_type(&self, name: &str, args: Vec<u32>) -> Result<NativeTypeInstance, ConnectorError> {
        ...
        let native_type = match name {
            ...
            VARCHAR_TYPE_NAME => PostgresType::VarChar(get_argument(args)),
            CHAR_TYPE_NAME => PostgresType::Char(get_argument(args)),
            TEXT_TYPE_NAME => PostgresType::Text,
            TIMESTAMP_TYPE_NAME => PostgresType::Timestamp(get_argument(args) as u8),
            TIMESTAMP_WITH_TIMEZONE_TYPE_NAME => PostgresType::TimestampWithTimeZone(get_argument(args) as u8),
            DATE_TYPE_NAME => PostgresType::Date,
            TIME_TYPE_NAME => PostgresType::Time(get_argument(args) as u8),
            TIME_WITH_TIMEZONE_TYPE_NAME => PostgresType::TimeWithTimeZone(length as u8),
            BOOLEAN_TYPE_NAME => PostgresType::Boolean,
            BIT_TYPE_NAME => PostgresType::Bit(get_argument(args)),
            VAR_BIT_TYPE_NAME => PostgresType::VarBit(get_argument(args)),
