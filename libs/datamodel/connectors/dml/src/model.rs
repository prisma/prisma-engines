...
pub struct IndexDefinition {
    ...
    pub fields: Vec<(String, Option<SortOrder>, Option<u32>)>,
    pub tpe: IndexType,
...
impl Model {
            ...
            let mut unique_field_combi: Vec<UniqueCriteria> = self
                ...
                .filter_map(|id| {
                    let fields: Vec<_> = id
                        .fields
                        .iter()
                        .map(|(f, _, _)| self.find_scalar_field(f).unwrap())
                        .collect();
    ...
    pub fn field_is_indexed(&self, field_name: &str) -> bool {
        ...
        let is_first_in_index = self
            .indices
            .iter()
            .any(|index| index.fields.first().unwrap().0 == field_name);

        let is_first_in_primary_key = matches!(&self.primary_key, Some(PrimaryKeyDefinition{ fields, ..}) if fields.first().unwrap().0 == field_name);
    ...
    pub fn field_is_primary(&self, field_name: &str) -> bool {
        matches!(&self.primary_key, Some(PrimaryKeyDefinition{ fields, .. }) if fields.len() ==1  && fields.first().unwrap().0 == field_name)
    }

    pub fn field_is_primary_and_defined_on_field(&self, field_name: &str) -> bool {
        matches!(&self.primary_key, Some(PrimaryKeyDefinition{ defined_on_field , ..}) if *defined_on_field && self.field_is_primary(field_name))
    }
