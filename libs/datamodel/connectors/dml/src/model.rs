...
impl Model {
    ...
    fn unique_criterias(&self, allow_optional: bool, disregard_unsupported: bool) -> Vec<UniqueCriteria> {
        ...
        {
            if let Some(x) = self.singular_id_fields() {
    ...
    pub fn singular_id_fields(&self) -> Option<&ScalarField> {
        (self.id_fields.len() == 1).then(|| self.find_scalar_field(self.id_fields.first().unwrap()).unwrap())
    }
    ...
    pub fn has_single_id_field(&self) -> bool {
        self.singular_id_fields().is_some()
    }
    ...
    pub fn field_is_primary(&self, name: &str) -> bool {
        self.id_fields == vec![name.to_string()]
    }

    // fn field_is_auto_generated_int_id(&self, name: &str) -> bool {
    //     let field = self.find_scalar_field(name).unwrap();
    //
    //     let is_autogenerated_id =
    //         matches!(field.default_value, Some(DefaultValue::Expression(_)) if self.field_is_primary(name));
    //
