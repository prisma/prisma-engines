...
mod shard_crud {
    ...
    // Schema for testing various CRUD operations with shard keys
    ...
    async fn create_one_customer(runner: Runner) -> TestResult<()> {
        ...
        assert!(result.contains("customer-1"));
    ...
    async fn create_customer_with_nested_profile(runner: Runner) -> TestResult<()> {
        ...
        assert!(result.contains("customer-1"));
        assert!(result.contains("profile-1"));
    ...
    async fn find_first_customer_with_shard_filter(runner: Runner) -> TestResult<()> {
        ...
        assert!(result.contains("us-east-1"));
        assert!(result.contains("findFirstCustomer"));
    ...
    async fn composite_shard_key_product_ops(runner: Runner) -> TestResult<()> {
        ...
        assert!(find_result.contains("product-1"));
        assert!(find_result.contains("Gaming Laptop"));
