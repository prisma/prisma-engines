...
mod int_id_update {
    ...
    fn schema_int_autoinc_cockroachdb() -> String {
        let schema = indoc! {
            r#"model Todo {
    ...
    // "Updating an item with an id field of type Int with autoincrement" should "work"
    #[connector_test(schema(schema_int_autoinc_cockroachdb), only(CockroachDb))]
    async fn update_id_int_autoinc_cockroachdb(runner: Runner) -> TestResult<()> {
        // Setup
        insta::assert_snapshot!(
          run_query!(&runner, r#"mutation {
            createOneTodo(data: {title: "initial"}) {title, id}
    ...
    fn schema_int_non_uniq_autoinc_cockroachdb() -> String {
        let schema = indoc! {
            r#"model Todo {
              #id(id, String, @id)
              counter    Int @default(sequence())
              title      String
            }"#
        };
    ...
    // "Updating a non-unique field of type Int with autoincrement" should "work"
    #[connector_test(schema(schema_int_non_uniq_autoinc_cockroachdb), only(CockroachDb))]
    async fn update_non_uniq_int_field_autoinc_cockroachdb(runner: Runner) -> TestResult<()> {
        // Setup
        insta::assert_snapshot!(
          run_query!(&runner, r#"mutation {
            createOneTodo(data: {id: "the-id", title: "initial"}) {title, id, counter}
