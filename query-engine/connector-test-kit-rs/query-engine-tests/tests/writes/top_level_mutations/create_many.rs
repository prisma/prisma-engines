...
mod create_many {
    ...
    async fn create_many_with_many_fields(runner: Runner) -> TestResult<()> {
        ...
        let max_bind_values = runner
            .max_bind_values()
            .expect("QUERY_BATCH_SIZE env var must be set during testing");

        assert!(max_bind_values < FIELDS_IN_TEST_MODEL, "QUERY_BATCH_SIZE value must be less than {FIELDS_IN_TEST_MODEL}. Either update this test, or the QUERY_BATCH_SIZE env var value in .envrc and GitHub Actions pipelines");
