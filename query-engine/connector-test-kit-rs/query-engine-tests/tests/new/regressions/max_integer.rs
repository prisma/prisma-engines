...
mod max_integer {
    ...
    #[connector_test(schema(overflow_mongodb), only(MongoDb))]
    ...
    #[connector_test(schema(overflow_mongodb), only(MongoDb))]
    async fn fitted_int_should_work_mongodb(runner: Runner) -> TestResult<()> {
        // int
        insta::assert_snapshot!(
          run_query!(&runner, format!("mutation {{ createOneTest(data: {{ id: 1, int: {} }}) {{ id int }} }}", i32::MAX)),
          @r###"{"data":{"createOneTest":{"id":1,"int":2147483647}}}"###
        );
        insta::assert_snapshot!(
          run_query!(&runner, format!("mutation {{ createOneTest(data: {{ id: 2, int: {} }}) {{ id int }} }}", i32::MIN)),
          @r###"{"data":{"createOneTest":{"id":2,"int":-2147483648}}}"###
        );
