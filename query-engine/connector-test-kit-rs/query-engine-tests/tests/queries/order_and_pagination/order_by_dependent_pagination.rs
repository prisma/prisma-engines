...
mod order_by_dependent_pag {
    ...
    async fn hop_1_related_record_asc_nulls(runner: Runner) -> TestResult<()> {
        ...
        match_connector_result!(
            ...
            MongoDb(_) | Sqlite | MySql(_) | CockroachDb | TiDB => vec![r#"{"data":{"findManyModelA":[{"id":1,"b":{"id":1}},{"id":2,"b":{"id":2}}]}}"#],
    ...
    async fn hop_2_related_record_asc_null(runner: Runner) -> TestResult<()> {
        ...
        match_connector_result!(
            ...
            // Depends on how null values are handled.
            MongoDb(_) | Sqlite | MySql(_) | CockroachDb | TiDB => vec![r#"{"data":{"findManyModelA":[{"id":1,"b":{"c":{"id":1}}}]}}"#],
    ...
    async fn circular_diff_related_record_asc(runner: Runner) -> TestResult<()> {
        ...
        match_connector_result!(
            ...
            // Depends on how null values are handled.
            MongoDb(_) | MySql(_) | Sqlite | CockroachDb | TiDB => vec![r#"{"data":{"findManyModelA":[{"id":1,"b":{"c":{"a":{"id":3}}}},{"id":2,"b":{"c":{"a":{"id":4}}}}]}}"#],
