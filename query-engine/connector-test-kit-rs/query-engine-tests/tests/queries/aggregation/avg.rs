...
mod aggregation_avg {
    ...
    async fn avg_with_all_sorts_of_query_args(runner: &Runner) -> TestResult<()> {
        ...
        insta::assert_snapshot!(
            run_query!(runner, "query { aggregateTestModel(take: 2) { avg { int bInt float decimal } } }"),
            @r###"{"data":{"aggregateTestModel":{"avg":{"int":7.5,"bInt":7.5,"float":5.0,"decimal":"5"}}}}"###
        );

        insta::assert_snapshot!(
            run_query!(runner, "query { aggregateTestModel(take: 5) { avg { int bInt float decimal } } }"),
            @r###"{"data":{"aggregateTestModel":{"avg":{"int":4.5,"bInt":4.5,"float":2.875,"decimal":"2.875"}}}}"###
        );
        ...
        insta::assert_snapshot!(
            run_query!(runner, r#"query { aggregateTestModel(where: { id: { gt: 2 }}) { avg { int bInt float decimal } } }"#),
            @r###"{"data":{"aggregateTestModel":{"avg":{"int":1.5,"bInt":1.5,"float":0.75,"decimal":"0.75"}}}}"###
        );

        insta::assert_snapshot!(
            run_query!(runner, "query { aggregateTestModel(skip: 2) { avg { int bInt float decimal } } }"),
            @r###"{"data":{"aggregateTestModel":{"avg":{"int":1.5,"bInt":1.5,"float":0.75,"decimal":"0.75"}}}}"###
        );
