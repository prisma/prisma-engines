...
impl From<ApiError> for user_facing_errors::Error {
    fn from(err: ApiError) -> Self {
        match err {
            ...
            ApiError::Conversion(full_error, _) => user_facing_errors::Error::from(
...
impl ApiError {
    pub fn conversion(diagnostics: impl ToString, dml: impl ToString) -> Self {
        Self::Conversion(diagnostics.to_string(), dml.to_string())
    }
