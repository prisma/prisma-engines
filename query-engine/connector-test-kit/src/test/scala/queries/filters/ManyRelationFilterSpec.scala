...
class ManyRelationFilterSpec extends FlatSpec with Matchers with ApiSpecBase {
  ...
  "1 level m-relation filter" should "work for `some`" in {
    ...
    server.query(query = """{blogs(where:{posts: { some:{AND:[{title: "post 1"}, {title: "post 2"}]}}}){name}}""", project = project).toString should be(
      """{"data":{"blogs":[]}}""")

    server
      .query(query = """{blogs(where:{AND:[{posts: { some:{title: "post 1"}}}, {posts: { some:{title: "post 2"}}]}}){name}}""", project = project)
      .toString should be("""{"data":{"blogs":[{"name":"blog 1"}]}}""")

    server.query(query = """{blogs(where:{posts: { some:{AND:[{title: "post 1"}, {popularity: { gte: 2 }}}]}}){name}}""", project = project).toString should be(
  ...
  "2 level m-relation filter" should "work for some/some" in {
    ...
    // some|some
    server.query(query = """{blogs(where:{posts: { some:{comments: { some: {likes: 0}}}}}){name}}""", project = project).toString should be(
      """{"data":{"blogs":[{"name":"blog 1"}]}}""")

    server.query(query = """{blogs(where:{posts: { some:{comments: { some: {likes: 1}}}}}){name}}""", project = project).toString should be(
      """{"data":{"blogs":[]}}""")
  }
  ...
  "2 level m-relation filter" should "work for `every`, `some` and `none`" taggedAs (IgnoreMongo) in {
    ...
    server.query(query = """{blogs(where:{posts: { some:{comments: { every: {likes: 0}}}}}){name}}""", project = project).toString should be(
      """{"data":{"blogs":[]}}""")

    // some|none
    server
      .query(query = """{blogs(where:{posts: { some:{comments: { none: {likes: 0}}}}},orderBy: { id: asc }){name}}""", project = project)
      .toString should be("""{"data":{"blogs":[{"name":"blog 1"},{"name":"blog 2"}]}}""")
    ...
    // every|some
    server.query(query = """{blogs(where:{posts: { every:{comments: { some: {likes: 10}}}}}){name}}""", project = project).toString should be(
      """{"data":{"blogs":[{"name":"blog 1"}]}}""")

    server.query(query = """{blogs(where:{posts: { every:{comments: { some: {likes: 0}}}}}){name}}""", project = project).toString should be(
      """{"data":{"blogs":[]}}""")
    ...
    server.query(query = """{blogs(where:{posts: { every:{comments: { every: {likes: 0}}}}}){name}}""", project = project).toString should be(
      """{"data":{"blogs":[]}}""")
    ...
    server.query(query = """{blogs(where:{posts: { every:{comments: { none: {likes: 0}}}}}){name}}""", project = project).toString should be(
      """{"data":{"blogs":[{"name":"blog 2"}]}}""")
    ...
    server.query(query = """{blogs(where:{posts: { none:{comments: { some: {likes: 0}}}}}){name}}""", project = project).toString should be(
      """{"data":{"blogs":[{"name":"blog 2"}]}}""")
