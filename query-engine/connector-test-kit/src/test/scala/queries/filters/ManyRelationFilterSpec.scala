...
class ManyRelationFilterSpec extends FlatSpec with Matchers with ApiSpecBase {
  ...
  "1 level m-relation filter" should "work for _some" in {
    ...
    server.query(query = """{blogs(where:{posts_some:{popularity: { gte: 5 }}},orderBy: { id: asc }){name}}""", project = project).toString should be(
      """{"data":{"blogs":[{"name":"blog 1"},{"name":"blog 2"}]}}""")

    server.query(query = """{blogs(where:{posts_some:{popularity: { gte: 50 }}}){name}}""", project = project).toString should be(
    ...
    server.query(query = """{blogs(where:{posts_some:{AND:[{title: "post 1"}, {popularity: { gte: 2 }}]}}){name}}""", project = project).toString should be(
      """{"data":{"blogs":[{"name":"blog 1"}]}}""")
  }

  "1 level m-relation filter" should "work for _every " taggedAs (IgnoreMongo) in {
    server.query(query = """{blogs(where:{posts_every:{popularity: { gte: 2 }}},orderBy: { id: asc }){name}}""", project = project).toString should be(
      """{"data":{"blogs":[{"name":"blog 1"},{"name":"blog 2"}]}}""")

    server.query(query = """{blogs(where:{posts_every:{popularity: { gte: 3 }}}){name}}""", project = project).toString should be(
      """{"data":{"blogs":[{"name":"blog 2"}]}}""")
  }

  "1 level m-relation filter" should "work for _none" taggedAs (IgnoreMongo) in {
    server.query(query = """{blogs(where:{posts_none:{popularity: { gte: 50 }}}){name}}""", project = project).toString should be(
      """{"data":{"blogs":[{"name":"blog 1"}]}}""")

    server.query(query = """{blogs(where:{posts_none:{popularity: { gte: 5 }}}){name}}""", project = project).toString should be("""{"data":{"blogs":[]}}""")
  }
  ...
  "2 level m-relation filter" should "work for _every, _some and _none" taggedAs (IgnoreMongo) in {
    // some|every
    server
      .query(query = """{blogs(where:{posts_some:{comments_every: {likes: { gte: 0 }}}},orderBy: { id: asc }){name}}""", project = project)
    ...
    server.query(query = """{blogs(where:{posts_some:{comments_none: {likes: { gte: 0 }}}}){name}}""", project = project).toString should be(
      """{"data":{"blogs":[]}}""")
    ...
    // every|every
    server
      .query(query = """{blogs(where:{posts_every:{comments_every: {likes: { gte: 0 }}}},orderBy: { id: asc }){name}}""", project = project)
      .toString should be("""{"data":{"blogs":[{"name":"blog 1"},{"name":"blog 2"}]}}""")
    ...
    // every|none
    server.query(query = """{blogs(where:{posts_every:{comments_none: {likes: { gte: 100 }}}}){name}}""", project = project).toString should be(
      """{"data":{"blogs":[{"name":"blog 1"}]}}""")
    ...
    // none|some
    server.query(query = """{blogs(where:{posts_none:{comments_some: {likes: { gte: 100 }}}}){name}}""", project = project).toString should be(
      """{"data":{"blogs":[{"name":"blog 1"}]}}""")
    ...
    // none|every
    server.query(query = """{blogs(where:{posts_none:{comments_every: {likes: { gte: 11 }}}}){name}}""", project = project).toString should be(
      """{"data":{"blogs":[{"name":"blog 1"}]}}""")

    server.query(query = """{blogs(where:{posts_none:{comments_every: {likes: { gte: 0 }}}}){name}}""", project = project).toString should be(
      """{"data":{"blogs":[]}}""")

    // none|none
    server
      .query(query = """{blogs(where:{posts_none:{comments_none: {likes: { gte: 0 }}}},orderBy: { id: asc }){name}}""", project = project)
      .toString should be("""{"data":{"blogs":[{"name":"blog 1"},{"name":"blog 2"}]}}""")

    server.query(query = """{blogs(where:{posts_none:{comments_none: {likes: { gte: 11 }}}}){name}}""", project = project).toString should be(
      """{"data":{"blogs":[{"name":"blog 2"}]}}""")
  }
  ...
  "Join Relation Filter on many to many relation" should "work on one level" taggedAs (IgnorePostgres) in {
    ...
    testDataModels.testV11 { project =>
      ...
      val res = server.query(
        """query{aUsers(where:{name: { starts_with: "Author2" }, posts_some:{title: { ends_with: "1" }}},orderBy: { id: asc }){name, posts(orderBy: { id: asc }){title}}}""",
        project
      )
