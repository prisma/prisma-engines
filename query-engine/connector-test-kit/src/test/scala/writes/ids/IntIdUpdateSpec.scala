...
class IntIdUpdateSpec extends FlatSpec with Matchers with ApiSpecBase {
  ...
  "Updating the id of an item with an id field of type Int with autoincrement" should "messages" in {
    val project = ProjectDsl.fromString {
      s"""
         |model Todo {
         |  id    Int @id @default(autoincrement())
  ...
  "Updating a unique field of type Int with autoincrement" should "messages" taggedAs (IgnoreSQLite, IgnoreMySql) in {
    val project = ProjectDsl.fromString {
      s"""
         |model Todo {
         |  id         String @id
