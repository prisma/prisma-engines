package writes.withCompoundId.nestedMutations.byId
  ...
  "a P1! to C1! relation with the child already in a relation" should "error since old required parent relation would be broken" in {
    schemaP1reqToC1reqWithCompoundId.test { dataModel =>
      ...
      val child1Id = server
        ...
        .pathAsString("data.createParent.childReq")
      ...
      val parentId2 = server
        ...
        .pathAsString("data.createParent")
      ...
      server.queryThatMustFail(
        s"""mutation {
           |  updateParent(
           |  where: {id_1_id_2: {$parentId2}}
           |  data:{
           |    p: "p2"
           |    childReq: {connect: {id_1_id_2: {$child1Id}}}
           |  }){
           |    childReq {
           |      c
           |    }
