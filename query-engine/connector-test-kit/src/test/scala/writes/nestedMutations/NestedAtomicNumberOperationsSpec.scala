...
class NestedAtomicNumberOperationsSpec extends FlatSpec with Matchers with ApiSpecBase with SchemaBaseV11 {
  ...
  "A nested updateOne mutation" should "correctly apply all number operations for Float" in {
    ...
    queryNestedNumberOperation(project, 2, "optFloat", "decrement", "4.6") should be(s"""{"optFloat":5.5}""")
    ...
    queryNestedNumberOperation(project, 2, "optFloat", "divide", "2") should be(s"""{"optFloat":5.5}""")

    // Set
    queryNestedNumberOperation(project, 1, "optFloat", "set", "5.1") should be(s"""{"optFloat":5.1}""")
    queryNestedNumberOperation(project, 2, "optFloat", "set", "5.1") should be(s"""{"optFloat":5.1}""")
