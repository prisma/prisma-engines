    ...
    fn from(re: RawError) -> SqlError {
        match re {
            ...
            RawError::UnsupportedColumnType { column_type } => Self::RawError {
                ...
                message: format!(
                    r#"Failed to deserialize column of type '{}'. If you're using $queryRaw and this column is explicitly marked as `Unsupported` in your datamodel, try casting this column to any supported Prisma type such as `String`."#,
                    column_type
                ),
            },
