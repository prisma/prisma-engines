...
impl QueryGraph {
    ...
    /// Traverses the query graph and checks if reloads of changed relations are necessary for the result node.
    /// This is a specialized case of reloading, as it checks the following:
    /// If the result query depends on a parent model projection that contains a relation field that is inlined on the parent model,
    /// and that relation mapping changes during the query graph execution (e.g. a nested mutation like connect changes the inlined
    /// relation value), we need to reload the relation in order to satisfy the result node.
    /// Assumptions:
    /// - Result nodes only have one parent.
    ///
    /// -----
    /// Alternative ideas:
    /// - Rewire result node (remove / reinsert or new edges).
    /// - Keep runtime query context that knows when the result query needs to change.
    /// - Are swaps possible?
    fn insert_result_relation_reloads(&mut self) -> QueryGraphResult<()> {
        // let reloads: Vec<(NodeRef, ModelRef, Vec<(EdgeRef, ModelProjection)>)> = self
        //     .result_nodes
        //     .iter()
        //     .filter_map(|ix| {
        //         let result_node = NodeRef { node_ix: *ix };

        //         if let Node::Query(Query::Read(ReadQuery::RecordQuery(q))) = self.node_content(&result_node).unwrap() {
        //             // let edges = self.outgoing_edges(&node);
        //             let mut parent_edges = self.incoming_edges(&result_node);
        //             let parent_edge = parent_edges.pop().unwrap();
        //             let model_id = q.model.primary_identifier();

        //             // Check selected fields for inlined relations &&
        //             // Check if the relation is part of the primary ID.
        //             let selected_relations: Vec<_> = q
        //                 .selected_fields
        //                 .relation
        //                 .iter()
        //                 .filter_map(|rf| {
        //                     if rf.field.is_inlined_on_enclosing_model() && model_id.contains(rf.field.clone()) {
        //                         Some(rf.field.clone())
        //                     } else {
        //                         None
        //                     }
        ...
        //             // For every relation, extract a selector for the related record from the nested operations, if any.
        //             // Traverse the graph beginning from the parent and ignore everything but write queries.
        //             // In the write queries, check if the linking fields of the relation for the child model are touched.
        //             // This can either happen by connect, update, create, upsert
