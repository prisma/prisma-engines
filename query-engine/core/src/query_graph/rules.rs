...
fn if_flow_edge_rules(graph: &QueryGraph, edge: &EdgeRef)  -> QueryGraphResult<()> {
    ...
    if let Node::Flow(f @ Flow::If(_)) = source_node_content {
        match graph.edge_content(edge).unwrap() {
            QueryGraphDependency::Then | QueryGraphDependency::Else => Ok(()),
            x => Err(QueryGraphError::RuleViolation(format!("Invalid edge '{}' for If node {}.", x, source_node.id()))),
        }?;

        let pairs = graph.child_pairs(&source_node);
        if pairs.len() > 2 {
            return Err(QueryGraphError::RuleViolation("'If' node has invalid amound of children (min 1, max 2).".into()));
        }
    }
