...
use connector::{Filter, AllOperations, WriteArgs};
...
async fn set<'a, 'b>(tx: &'a Box<dyn AllOperations<'b> + 'b>, q: SetRecords) -> InterpretationResult<QueryResult> {
    tx.set(
        q.relation_field,
...
async fn reset<'a, 'b>(_tx: &'a Box<dyn AllOperations<'b> + 'b>, _q: ResetData) -> InterpretationResult<QueryResult> {
    unimplemented!()
}
