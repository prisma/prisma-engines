...
async fn sqlite(
    ...
    let mut sqlite = Sqlite::from_source(source, url).await?;

    sqlite.set_features(features);
...
async fn postgres(
    ...
    let mut psql = PostgreSql::from_source(source, url).await?;

    psql.set_features(features);
...
async fn mysql(
    ...
    let mut mysql = Mysql::from_source(source, url).await?;

    mysql.set_features(features);
...
async fn mssql(
    ...
    let mut mssql = Mssql::from_source(source, url).await?;

    mssql.set_features(features);
