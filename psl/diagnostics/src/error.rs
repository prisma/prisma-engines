...
impl DatamodelError {
    ...
    pub fn new_datasource_direct_url_removed_error(span: Span) -> DatamodelError {
        let msg = "The datasource property `directUrl` is no longer supported in schema files. Move connection URLs to `prisma.config.ts`. See http://pris.ly/d/config-datasource";
        Self::new(msg.to_string(), span)
    }

    pub fn new_datasource_shadow_database_url_removed_error(span: Span) -> DatamodelError {
        let msg = "The datasource property `shadowDatabaseUrl` is no longer supported in schema files. Move connection URLs to `prisma.config.ts`. See http://pris.ly/d/config-datasource";
        Self::new(msg.to_string(), span)
    }
