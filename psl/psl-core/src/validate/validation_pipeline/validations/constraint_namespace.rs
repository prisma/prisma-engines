use crate::datamodel_connector::{walker_ext_traits::*, Connector, ConstraintScope};
use parser_database::{ast, ParserDatabase};
...
impl<'db> ConstraintNamespace<'db> {
    ...
    pub(super) fn add_local_custom_names_for_primary_keys_and_uniques(&mut self, db: &'db ParserDatabase) {
        for model in db.walk_models() {
