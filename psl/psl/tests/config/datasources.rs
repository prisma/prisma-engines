...
fn parse_direct_url_should_error() {
    ...
    let expect = expect![[r#"
        [1;91merror[0m: [1mThe datasource property `directUrl` is no longer supported in schema files. Move connection URLs to `prisma.config.ts`. See http://pris.ly/d/config-datasource[0m
